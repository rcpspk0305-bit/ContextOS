...
| **`assistant-ui/tool-ui`** | https://github.com/assistant-ui/tool-ui | `@assistant-ui/react` (Latest npm) | `packages/react/src/components/ToolCall.tsx` | `web/components/approval/approval-card.tsx` | MIT | Tool-call and argument rendering card pattern | Embedded human-in-the-loop approval actions (`Approve once`, `Approve for session`, `Reject`); styled with `shadcn/ui` tokens. |
| **`shadcn-ui/ui`** | https://github.com/shadcn-ui/ui | Latest CLI templates | Various primitives (button, card, dialog, tabs, sheet, badge) | `web/components/ui/*` | MIT | Tailwind CSS + Radix UI accessible dashboard component primitives | Styled using custom neutral slate / zinc tokens; dark/light mode optimized. |
